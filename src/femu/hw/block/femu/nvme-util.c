        ...
        uint16_t ctrl, uint64_t data_size, uint64_t meta_size)
{
    ...
    if (n->id_ctrl.mdts && data_size > n->page_size * (1 << n->id_ctrl.mdts)) {
        ...
        printf("error nvme_rw_check_req data_size, op=%d, slba=%lu, elba=%lu, nlb=%u, ctrl=%u, data_size=%lu, meta_size=%lu\n",
            req->opcode, slba, elba, nlb, ctrl, data_size, meta_size);
        return NVME_INVALID_FIELD | NVME_DNR;
    }
    if (meta_size) {
        nvme_set_error_page(n, req->sq->sqid, cmd->cid, NVME_INVALID_FIELD,
                offsetof(NvmeRwCmd, control), ctrl, ns->id);
        printf("error nvme_rw_check_req meta_size, op=%d, slba=%lu, elba=%lu, nlb=%u, ctrl=%u, data_size=%lu, meta_size=%lu\n",
            req->opcode, slba, elba, nlb, ctrl, data_size, meta_size);
        return NVME_INVALID_FIELD | NVME_DNR;
    }
    if ((ctrl & NVME_RW_PRINFO_PRACT) && !(ns->id_ns.dps & DPS_TYPE_MASK)) {
        nvme_set_error_page(n, req->sq->sqid, cmd->cid, NVME_INVALID_FIELD,
                offsetof(NvmeRwCmd, control), ctrl, ns->id);
        printf("error nvme_rw_check_req ctrl, op=%d, slba=%lu, elba=%lu, nlb=%u, ctrl=%u, data_size=%lu, meta_size=%lu\n",
            req->opcode, slba, elba, nlb, ctrl, data_size, meta_size);
