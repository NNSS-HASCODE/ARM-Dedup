    ...
    NvmeRequest *req)
{
    ...
    if (dw11 & NVME_DSMGMT_AD) {
        ...
        for (i = 0; i < nr; i++) {
            ...
            // uint32_t lastSSD_id = ((NvmeDsmCmd *)cmd)->rsvd12[2];
            // req->slba = slba;
            // req->is_remote_slba = (tgtSSD_id != defaultSSD_id);
            // req->global_slba = defaultSSD_id;
            // req->special_value = false;

            // if(tgtSSD_id == -1) {
            //     req->special_value = true;  //-1:is a Init cmd
            //     if(defaultSSD_id == -1)
            //         req->global_slba = -1;   //Test begin
            //     else if(defaultSSD_id == -2)
            //         req->global_slba = -2;   //Test end
            //     else
            //         req->global_slba = defaultSSD_id;   //R-MapTable Size
            //     return NVME_SUCCESS;
            // }
            if (slba + nlb > le64_to_cpu(ns->id_ns.nsze)) {
                ...
                printf("TRIM error in FEMU_memory, is_remote=%d, slba=%lu, entry_offset=%lu\n", 
                                        dsm_cmd->rsvd12[0], dsm_cmd->rsvd2[0], dsm_cmd->rsvd12[1]);
